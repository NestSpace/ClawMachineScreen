"""Protected settings panel that requires admin authentication."""
...
class Panel(ScreenPanel):
    def __init__(self, screen, title):
        ...
        options.append({"lang": {"name": _("Language"), "type": "menu", "menu": "lang"}})
        ...
        self.labels["settings_menu"].set_policy(Gtk.PolicyType.NEVER, Gtk.PolicyType.AUTOMATIC)
        self.labels["lang_menu"].set_policy(Gtk.PolicyType.NEVER, Gtk.PolicyType.AUTOMATIC)
        self.labels["printers_menu"].set_policy(Gtk.PolicyType.NEVER, Gtk.PolicyType.AUTOMATIC)
    ...
    def _check_auth(self):
        ...
        logging.info("Settings protected: _check_auth called")

        def on_auth_success(api_key, role):
            logging.info(f"Settings access granted to {role}")
            # Auth successful, user can now use settings

        def on_auth_cancel():
            logging.info("Settings access denied - going back")
            # User cancelled or failed auth, go back
            self._screen._menu_go_back(None, home=False)

        # Show escalate screen requiring admin or manager role
        logging.info("Settings protected: calling escalate_screen.escalate")
